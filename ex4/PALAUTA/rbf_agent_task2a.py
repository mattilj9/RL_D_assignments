...
qs_actions = env.action_space.n


class Agent(object):
    def __init__(self, num_qs_actions, gamma=0.98, memory_size=5000, batch_size=32):
        self.scaler = None
        self.featurizer = None
        self.q_functions = None
        self.gamma = gamma
        self.batch_size = batch_size
        self.num_qs_actions = num_qs_actions
    ...
    def initialize_model(self):
        ...
        self.q_functions = [SGDRegressor(learning_rate="constant", max_iter=500, tol=1e-3)
                       for _ in range(self.num_qs_actions)]
    ...
    def get_action(self, state, epsilon=0.0):
        ...
        qs_actions = np.zeros(self.num_qs_actions)
        for aidx in np.arange(self.num_qs_actions):
            qs_actions[aidx] = self.q_functions[aidx].predict(featurized_state)
           
        max_action_index = np.argmax(qs_actions)
        action_index = max_action_index

        # random float in the half-open interval [0.0, 1.0)
        if np.random.random_sample() < epsilon:         
            action_index = np.random.randint(self.num_qs_actions)
    ...
    def single_update(self, state, action, next_state, reward, done):
        ...
        qs_actions = np.zeros(self.num_qs_actions)
        for aidx in np.arange(self.num_qs_actions):
            qs_actions[aidx] = self.q_functions[aidx].predict(featurized_next_state)

        next_qs = np.max(qs_actions)
        if done:
            next_qs = 0
        
        # Calculate the updated target Q- values
        # Task 1: TODO: Calculate target based on rewards and next_qs
        target = np.array([reward + self.gamma * next_qs])
    ...
    def update_estimator(self):
        ...
        featurized_next_states = [ self.featurize(next_state) for next_state in next_states ]
        num_samples = len(featurized_next_states)

        next_qs = [] 
        for sidx in np.arange(num_samples):            
            qs_actions = np.zeros(self.num_qs_actions)
            for aidx in np.arange(self.num_qs_actions):
                qs_actions[aidx] = self.q_functions[aidx].predict(featurized_next_states[sidx])

            if dones[sidx]:
                next_qs.append(0.0)
            else:
                next_qs.append(np.max(qs_actions))

        # Calculate the updated target values
        # Task 2: TODO: Calculate target based on rewards and next_qs
        #targets = 0
        targets = [ np.array([rewards[sidx] + self.gamma * next_qs[sidx]]) for sidx in np.arange(num_samples) ]
        ...
        for a in range(self.num_qs_actions):
            # Find states where a was taken
            idx = action == a

            # If a not present in the batch, skip and move to the next action
            if np.any(idx):
                #indexes = np.where(idx==True).flatten()
                indexes = np.arange(num_samples)[idx]
