...
class SailingGridworld(object):
    ...
    def value_iteration_task3(self, value_est, policy, gamma=0.9, epsilon = 10e-4):
        counter = 0
        while True:
            # store previous value       
            value_est_prev = np.array(value_est)
            #policy_prev = np.array(policy)
            ...
            delta = np.max(np.abs(value_est - value_est_prev))   
            #delta = np.max(np.abs(policy - policy_prev))
            counter = counter + 1
 
            # check convergence as max(|Vk(s)-Vk-1(s)| < e)
            if delta < epsilon:
                print("# of iterations: {}".format(counter))
                break
